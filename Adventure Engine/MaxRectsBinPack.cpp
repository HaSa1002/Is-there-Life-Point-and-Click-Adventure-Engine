...
void MaxRectsBinPack::Insert(std::vector<RectSize> &rects, std::vector<Rect> &dst, FreeRectChoiceHeuristic method)
{
	...
	while(rects.size() > 0)
	{
		...
		for(size_t i = 0; i < rects.size(); ++i)
		{
			int score1;
			int score2;
			Rect newNode = ScoreRect(rects[i].width, rects[i].height, rects[i].name, method, score1, score2);
...
Rect MaxRectsBinPack::ScoreRect(int width, int height, size_t name, FreeRectChoiceHeuristic method, int &score1, int &score2) const
{
	Rect newNode;
...
/// Computes the area of the final rect
sf::Vector2u MaxRectsBinPack::Size() const {
	sf::Vector2u vec;
		for (size_t i = 0; i < usedRectangles.size(); ++i) {
			vec.x += usedRectangles[i].width;
			vec.y += usedRectangles[i].height;
	}
	return vec;
}
